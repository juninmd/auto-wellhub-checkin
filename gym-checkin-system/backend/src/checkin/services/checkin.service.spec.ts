...
describe("CheckinService", () => {
  ...
  it("should process check-in successfully", async () => {
    mockBiometricsService.identify.mockReturnValue(
      of({ success: true, student_id: "student-123" }),
    );
    ...
    expect(mockBiometricsService.identify).toHaveBeenCalledWith({
      biometric_base64: "base64data",
    });
    expect(mockStudentsService.validateStudentPlan).toHaveBeenCalledWith(
      "student-123",
    );
    expect(mockHardwareProvider.openTurnstile).toHaveBeenCalledWith(
      "student-123",
    );
  ...
  it("should throw UnauthorizedException if biometric identification fails", async () => {
    mockBiometricsService.identify.mockReturnValue(
      of({ success: false, error_message: "Not found" }),
    );

    await expect(service.processCheckin("invalid-data")).rejects.toThrow(
      UnauthorizedException,
    );
  ...
  it("should throw UnauthorizedException if student plan is invalid", async () => {
    mockBiometricsService.identify.mockReturnValue(
      of({ success: true, student_id: "student-456" }),
    );
    mockStudentsService.validateStudentPlan.mockResolvedValue(false);

    await expect(service.processCheckin("base64data")).rejects.toThrow(
      UnauthorizedException,
    );
    expect(mockHardwareProvider.openTurnstile).not.toHaveBeenCalled();
  });
  ...
  it("should throw InternalServerErrorException if hardware fails", async () => {
    mockBiometricsService.identify.mockReturnValue(
      of({ success: true, student_id: "student-789" }),
    );
    mockStudentsService.validateStudentPlan.mockResolvedValue(true);
